...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $orderQuery = clone $orderObject;

        $totalSales = $orderQuery->where(function ($query) {
        ...
        $todayOrders = $orderQuery->whereDate('created_at', Carbon::today())->count();

        $pendingOrder = $orderQuery->where('order_status', OrderStatus::PENDING->value)->count();

        $toPickupOrders = $orderQuery->where(function ($query) {
        ...
        $toDeliveryOrders = $orderQuery->where('order_status', OrderStatus::ON_THE_WAY->value)->count();
        ...
        for ($i = 1; $i <= 12; $i++) {
            ...
            $totalAmount = $orderQuery->where(function ($query) use ($month, $startDate, $endDate) {
